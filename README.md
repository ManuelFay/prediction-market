...
The SPA lives in `frontend/` and compiles into `app/frontend/` so FastAPI can
serve the built assets under `/static`.

### Local development

1. Start the API in one terminal: `uvicorn app.main:app --reload`
2. In another terminal, run the Vite dev server with API proxying:
   ```bash
   cd frontend
   npm install
   npm run dev
   ```
3. Visit http://localhost:4173/ for the hot-reloading UI (requests are proxied
   to the API on port 8000).

### Production build

To refresh the static files that ship with the Python app (or the Docker
image), rebuild the frontend and commit the generated assets:
...
The build emits into `app/frontend/` with an asset base of `/static/`, which is
already mounted by FastAPI.
